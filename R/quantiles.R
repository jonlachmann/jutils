...
#' @export
quantmean.list <- function (matlist, quantiles=c(0.05, 0.95), mean=TRUE, median=FALSE) {
...
#' @return A list containing the mean per element over all the matrices, together with the low and high quantiles selected
#' @export
quantmean.array <- function (array, dim=1, quantiles=c(0.05, 0.95), mean=TRUE, median=FALSE) {
  ...
  if (median) result$median <- apply(array, dims, median)
  return(result)
}
